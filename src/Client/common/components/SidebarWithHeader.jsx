...
const SidebarWithHeader = ({ userInfo, pageTitle, children }) => {
  ...
  const menuItems = [
    ...
    { name: "Appointments", icon: Calendar, route: "/client-appointments" },
    { name: "Profile", icon: User, route: "/client-profile" },
  ]

  // Update active item on route change
  useEffect(() => {
    // Match if the current path starts with the menu item's route
